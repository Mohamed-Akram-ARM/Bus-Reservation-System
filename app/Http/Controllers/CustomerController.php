...
class CustomerController extends Controller
{
    ...
    public function pendingRequestApi()
    {
        ...
        $pending = Reservation::with(['user', 'bus'])->latest();
        return DataTables::of($pending)
        
        
        ->addColumn('name', function ($row) {
        return $row->bus 
            ? '<a target="_blank" href="/bus/' . $row->bus->id . '">' . $row->bus->name . '</a>'
            : 'No Bus';
    })
    ...
    ->addColumn('seat_no', function ($row) {
        return $row->seat_no;
    })
    ...
    ->addColumn('actions', function ($row) {
        return '<a class="btn btn-primary btn-sm" href="customer-reservations/' . $row->id . '/approve">Approve</a>
                <a class="btn btn-danger btn-sm" href="customer-reservations/' . $row->id . '/reject">Reject</a>';
    })
    ->rawColumns(['name', 'actions', 'pending', 'rejected'])
    ->make(true);
    }
